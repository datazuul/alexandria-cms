...
public class FilePathFactory {
   ...
   * Each <code>Image</code> is represented by a file, filename "{imageId}.(jpg|jpeg|png|gif)", and the corresponding
   * XML-metadata-file. An Image is stored in a specific directory-path under the root dir of the repository: the image
   * specific directory-path follows the following rule:
   *
   * subdirectory-path is the imageId splitted in 2 digits subdirs (starting from imageId head), e.g.:<br>
   *
   * <pre>
  ...
  public static String getRootPathRepository(final DomainName dn) {
    String rootPathRepository = REPOSITORY_ROOT;
  ...
  /**
   * Provides a simple path logic.<br>
   * Each <code>Article</code> is represented by a file, filename "{articleId}.html". An Article is stored in a specific
   * directory-path under the root dir of the repository: the article specific directory-path follows the following
   * rule:
   *
   * subdirectory-path is the articleId splitted in 2 digits subdirs (starting from articleId head), e.g.: articleId =
   * 1, path = repositoryroot/articles/1.xml articleId = 9, path = repositoryroot/articles/9.xml articleId = 10, path =
   * repositoryroot/articles/10/10.xml articleId = 11, path = repositoryroot/articles/11/11.xml articleId = 101, path =
   * repositoryroot/articles/10/101.xml articleId = 110, path = repositoryroot/articles/11/110.xml articleId = 1000,
   * path = repositoryroot/articles/10/00/1000.xml articleId = 1001, path = repositoryroot/articles/10/01/1001.xml
   * articleId = 13425, path = repositoryroot/articles/13/42/13425.xml articleId = 13426795, path =
