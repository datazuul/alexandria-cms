...
//        http.csrf().disable();
        ...
        http.authorizeRequests().antMatchers("/api/**/*", "/cms/**/*", "/error*", "/oai*", "/object/**", "/login*", "/setup/**","/uploads/**").permitAll();
        // Needed for preview
        http.authorizeRequests().antMatchers("/resource/**", "/iiif/**", "/pages/preview/**").permitAll();
        // Assets must be accessible to all, too
        http.authorizeRequests().antMatchers("/favicon.ico", "/css/**", "/images/**", "/img/**", "/js/**", "/fonts/*", "/less/*").permitAll();
